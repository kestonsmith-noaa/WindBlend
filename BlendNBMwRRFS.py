...
LocalFS  = [ rwps_pr, rwps_hi, rwps_ak ,  rwps_na] #file names
VarFS    = [ 1.      , 1.    , 4.      ,  25.    ] # m m /s /s
LambdaFS = [ 50.     , 200.  , 300.    ,  1000.  ] #km
...
u0=np.asarray(data["uwnd"][:,:])
v0=np.asarray(data["vwnd"][:,:])
e0=np.asarray(data["tri"][:,:])
...
var0=100.+np.zeros((nn,nt)) # prior variance for BG field 
...
#    np0=len(ng0)
    ...
    for k in range(nt):
        ...
        if len(j)==1:
            ...
            u0[ng0,k]=u0[ng0,k]+(var0[ng0,k]/(var0[ng0,k]+varm[ng0]))*(um[ng0,j]-u0[ng0,k])
            v0[ng0,k]=v0[ng0,k]+(var0[ng0,k]/(var0[ng0,k]+varm[ng0]))*(vm[ng0,j]-v0[ng0,k])
